...
import com.pictures_annotator.application.picture.queries.dto.BoundingBoxDto;
...
public class GetAllQueryHandlerTest {
    ...
    private GetAllQueryHandler handler;

    @BeforeEach
    void setUp() {
        pictureRepository = mock(PictureRepository.class);
        pictureMapper = mock(PictureMapper.class);
        handler = new GetAllQueryHandler(pictureRepository, pictureMapper);
    }
    ...
    void handle_givenPictures_shouldReturnListOfPictureDtos() {
        ...
        // When
        List<PictureDto> result = handler.handle(new GetAllQuery());
